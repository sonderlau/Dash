...
- summarize papers in Chinese from arXiv metadata and abstracts via Xiaomi MiMo streaming chat,
...
- The project is MiMo-chat-only; do not add a generic LLM provider abstraction.
...
2. `scripts/enrich.py`
   - streams one `mimo-v2.6-flash` chat completion per paper that still needs a summary,
   - sends title, authors, categories, dates, comment, journal ref, DOI, abstract URL, abstract, and optional personal keywords,
   - parses the finished JSON into `tldr`, `motivation`, `method`, `result`, `conclusion`, `relevance_score`.
...
- `scripts/summarize.py` — streaming MiMo JSON request/parse helpers.
- `scripts/snapshot_writer.py` — debounced state writer used by summary workers.
...
- `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `MODEL_NAME`, `LANGUAGE`, `LLM_TIMEOUT_SECONDS`, `LLM_RETRY_TIMES` configure MiMo chat. Defaults: `https://api.xiaomimimo.com/v1` and `mimo-v2.6-flash`.
- `daily.yml` summarizes and publishes `docs/data` in the same job.
...
- Streaming MiMo chat with `trust_env=False`, `thinking.type=disabled`, and `response_format` JSON object.
